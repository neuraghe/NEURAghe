...
# Description:    this script processes the weigths and the biases to be suitables for HWCE         #                            
#                                                                                                   #
#####################################################################################################
...
for l in range (0, NUM_LAYERS):
  ...
  #                                                
  print tcolors.OKBLUE + "Data convertion..." + tcolors.ENDC
  if settings['data_type']=='d' or settings['data_type']=='f':
    w = [convert_float_to_short(i, QF) for i in w]
    b = [convert_float_to_short(i, QF) for i in b]
    ...
    w = [int(i) for i in w]
    b = [int(i) for i in b]
    print tcolors.OK + "\t[OK] Casting to int done!\n" + tcolors.ENDC
  ...
  if FS==5:
    for i in range (0, of_round):
      print_pb(i, of_round)
      if i<OF:
        for j in range (0, if_round):
          if j<IF:
            dispose_weights_5x5 (w_disp, (i*if_round + j)*28, w[(i*IF + j)*25:], b[i])
          else:
            zeros_fill_5x5 (w_disp, (i*if_round + j)*28, 28, b[i])      
      else:
        zeros_fill_5x5 (w_disp, (i*if_round)*28, 28*if_round, 0)
  ...
  if FS==3:  
    ...
    for o in range (0, of_round):
      print_pb(o, of_round)
      if o<OF:
        for i in range ( 0, if_round/12):
          for k in range (0, 3):
            for j in range(0, 4):
              for www in range (0, 9):
                if i*12+k*4+j<IF:
                  w_disp[ o*if_round/12*4*28 + i*4*28 + j*28 + k*9 +www] = w[o*IF*9 + i*3*4*9 + k*4*9 + j*9 + www]
                else:
                  w_disp[ o*if_round/12*4*28 + i*4*28 + j*28 + k*9 +www] =0
      else:
        for www in range (0, 28*4*if_round/12):
          w_disp[ o*if_round/12*4*28 +www] = 0
      
    #ADDING BIASES
    for o in range (0, of_round):
      for i in range (0, if_round/3):
        if o<OF:
          w_disp[o*if_round/3*28 + i*28 + 27] = b[o];
        else:        
          w_disp[o*if_round/3*28 + i*28 + 27] = 0;
  ...
  #                                                |___/ 
  print tcolors.OKBLUE + "Interlacing..." + tcolors.ENDC
  with open(dest_file+".disp.pthex", "w") as f:
    for wb in w_disp:
      f.write("0x%x\n" % ctypes.c_uint32(wb).value)

  interlace(wb_inter,w_disp,WS,28)
  print tcolors.OK + "\t[OK] Interlacing done!\n" + tcolors.ENDC
  ...
  print "\t maxOG: %d\n" % maxOG  
  subdivide(wb_subd,wb_inter,IG,OG,maxOG,28)
