...
def zeros_fill (dest, offset, n, b): #FIXME
  for i in range (0, n):
    if i%n == n-1:
      dest[offset+i] =b
      #print "bias %d " % b
...
def dispose_weights (w_disp, offset, w_lin, bias, interl_stride):
  ...
  for o in range (0, of_round):
    ...
    if o<OF:
      for i in range (0, if_round/interl_stride):
        if i<IF:
          dispose_weights (w_disp, (o*if_round/interl_stride + i)*_WS_SIZE_*interl_stride, w[(o*IF/interl_stride + i)*(FS_h*FS_w*interl_stride):], b[o], interl_stride)
        else:
          zeros_fill (w_disp, (o*if_round + i)*_WS_SIZE_, _WS_SIZE_, b[o])      
    else:
      zeros_fill (w_disp, (o*if_round)*_WS_SIZE_, _WS_SIZE_*if_round, 0)
