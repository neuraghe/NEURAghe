...
def dispose_weights_5x5 (w_disp, offset, w_lin, bias):
  for i in range (0, 3):
   for j in range (0,12):
    if ((i*9+j)<25):
     w_disp[offset+i*12+j] = w_lin[i*9+j]
    else:
     w_disp[offset+i*12+j]=0
    ...
    for o in range (0, of_round):
      ...
      if o<OF:
        for i in range (0, if_round):
          if i<IF:
            dispose_weights_5x5 (w_disp, (o*if_round + i)*_WS_SIZE_*dim, w[(o*IF + i)*25:], b[o])
          else:
            zeros_fill_5x5 (w_disp, (o*if_round + i)*_WS_SIZE_*dim, _WS_SIZE_*dim, b[o])      
      else:
        zeros_fill_5x5 (w_disp, (o*if_round)*_WS_SIZE_*dim, _WS_SIZE_*if_round*dim, 0)
      ...
      if o<OF:
        for i in range ( 0, if_round):
              for www in range (0, 9):
                if i<IF:
                  w_disp[ o*IF*_WS_SIZE_ + i*_WS_SIZE_ +www] = w[o*IF*9 + i*9 + www]
                else:
                  w_disp[ o*IF*_WS_SIZE_ + i*_WS_SIZE_ +www] =0
      else:
        for www in range (0, _WS_SIZE_*IF):
          w_disp[ o*IF*_WS_SIZE_ +www] = 0
