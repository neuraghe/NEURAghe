...
PULP_L1_DATA unsigned int y_addr_hwce    [MAX_YS*2*2]; //MAX_YS*pingpong*circulararrayemulation
PULP_L1_DATA unsigned int y_in_addr_hwce [MAX_YS*2*2];

PULP_L1_DATA unsigned int y_addr         [MAX_YS*2*2]; 
PULP_L1_DATA unsigned int y_in_addr      [MAX_YS*2*2];
...
PULP_L1_DATA unsigned int xTILEs;
PULP_L1_DATA unsigned int yTILEs;
...
PULP_L1_DATA unsigned int conv_w;
...
int main() {
  ...
  _dbgm_printf_("Cluster id   : %d \n", *((volatile int*)CLUSTID_ADDR)); 
 ...
 */
       ...
       current_conv.y_ext = 0x80000000;    
         ...
         conv_w = (current_conv.precision8) ? 8 : 16;
         ...
         #ifdef NEU_TCN
            WS_SIZE   = ((current_conv.FS_h*current_conv.FS_w)+1);
         #else 
            WS_SIZE   = 32;
         #endif
         
         ws_size_B = WS_SIZE*(conv_w/8);
 ...
 _dbgm_printf_("sub_part_n %d\n", sub_part_n);        
         ...
         if (BANK_SLOT_SIZE_Y%8)
           BANK_SLOT_SIZE_Y = BANK_SLOT_SIZE_Y -BANK_SLOT_SIZE_Y%(8);
_dbgm_printf_("BANK_SLOT_SIZE_Y %d\n", BANK_SLOT_SIZE_Y);
         
         SLOT_SIZE_X = BYTE_PER_BANK/2;
_dbgm_printf_("SLOT_SIZE_X %d\n", SLOT_SIZE_X);
...
_dbgm_printf_("TILEs %d\n", TILEs);    
         ...
         //check tiles' size     for x slots    
         
         //check middle tiles
         int mid_tile_xh;
         
         do{                    
           mid_tile_xh = current_conv.IH/TILEs + (current_conv.FS_h-1)*current_conv.dil_h;
_dbgm_printf_("mid_tile_xh %d\n", mid_tile_xh);
...
_dbgm_printf_("TILEs %d\n", TILEs);
     ...
     //check bottom tiles 
        // int tile_xh= current_conv.IH/TILEs;
         int bottom_tile_xh;
         do{                    
           bottom_tile_xh = current_conv.IH - current_conv.IH/TILEs*(TILEs-1);
_dbgm_printf_("bottom_tile_xh %d\n", bottom_tile_xh);
           if (bottom_tile_xh*current_conv.IW*BxP > SLOT_SIZE_X)
             TILEs++;
         }while (bottom_tile_xh*current_conv.IW*BxP > SLOT_SIZE_X);
_dbgm_printf_("TILEs %d\n", TILEs); 

          int bottom_tile_yh;
         do{                    
           bottom_tile_yh = OH - OH/TILEs*(TILEs-1);
_dbgm_printf_("bottom_tile_yh %d\n", bottom_tile_yh);
           if (bottom_tile_yh*OW*BxP > BANK_SLOT_SIZE_Y)
             TILEs++;
         }while (bottom_tile_yh*OW*BxP > BANK_SLOT_SIZE_Y);
_dbgm_printf_("TILEs %d\n", TILEs); 
         ...
         unsigned int feat_per_IG;
            ...
            if(current_conv.IF<N_COL) 
              ...
              feat_per_IG = N_COL;
         #else
              feat_per_IG = (current_conv.FS_h==3) ? N_COL*3 : N_COL;
         #endif
    ...
    //     _dbgm_printf_("tile_xh: %d\n", tile_xh);
         ...
         */     
         ...
         if (current_conv.IH != 1 && current_conv.IH/TILEs + (current_conv.FS_h-1)*current_conv.dil_h < (current_conv.FS_h-1)*current_conv.dil_h + 1){
           printf ("ERROR! The tiling leads to a tile input height smaller than the filter size dilated.\n");  
           printf ("\t Try reducing MAX_OG value. Current value: %d\n", current_conv.MAX_OG);
         ...
         if (current_conv.IH != 1 && current_conv.zero_padding==0 && current_conv.IH - current_conv.IH/TILEs*(TILEs-1) < (current_conv.FS_h-1)*current_conv.dil_h + 1){
           printf ("ERROR! The tiling leads to a bottom tile input height smaller than the filter size dilated.\n");  
           printf ("\t Try reducing MAX_OG value. Current value: %d\n", current_conv.MAX_OG);
           
           conv_tasks[job_id].run = 103;
           job_id = (job_id+1)%MAX_JOBS;
          // *((volatile unsigned int*)(CONV_SETUP_ADDR      )) = (unsigned int)(conv_tasks);
         //  *((volatile unsigned int*)(CONV_SETUP_ADDR + 4*1)) = job_id;
           return 1;
         ...
         if (current_conv.MAX_OG*N_ROW*N_COL*WS_SIZE*2*2>WEI_MEM_SIZE_B){
           printf ("ERROR! The weights group size (%d B) execeded the available weights memory (%d B).\n", current_conv.MAX_OG*N_ROW*N_COL*WS_SIZE*2*2, WEI_MEM_SIZE_B);  
           printf ("\t Try reducing MAX_OG value. Current value: %d\n", current_conv.MAX_OG);
         ...
         if (YS>MAX_YS){
           printf ("ERROR! The numeber of y slots (%d B) execeded the maximum value (%d B).\n", YS, MAX_YS);  
           printf ("\t Try reducing MAX_OG value. Current value: %d\n", current_conv.MAX_OG);
           
           conv_tasks[job_id].run = 102;
           job_id = (job_id+1)%MAX_JOBS;
           return 1;
         }
   ...
   //printf("\n\nsetup:    %d\t  cycles\n\r", get_time());
         ...
         hwce_offload_ext ( current_conv.IH, current_conv.IW, current_conv.FS_h, current_conv.FS_w, current_conv.dil_h, current_conv.dil_w, feat_per_IG );  
    ...
    //   int tile_height = ih/TILEs; //unused
      ...
      */
        ...
        if (TILEs==1) { //only one tile
          ...
          tile.oh = (current_conv.zero_padding&0x1) ? o_tile_h                    : i_tile_h;
          tile.ih = (current_conv.zero_padding&0x1) ? o_tile_h + dil_h*(fw_h-1)/2 : i_tile_h + dil_h*(fw_h-1);
          
        
        }
        else if (t == TILEs-1){ //bottom tile
          o_tile_h = o_tile_h + (oh_tot - o_tile_h * TILEs);
          i_tile_h = i_tile_h + (ih_tot - i_tile_h * TILEs);
        
          tile.nb_nt_zp=0b010 + (current_conv.zero_padding&0x1);
          
          tile.oh = (current_conv.zero_padding&0x1) ? o_tile_h                    : i_tile_h - dil_h*(fw_h-1);
          tile.ih = (current_conv.zero_padding&0x1) ? o_tile_h + dil_h*(fw_h-1)/2 : i_tile_h /*+ dil_h*(fw_h-1)*/;
          ...
          tile.oh = (current_conv.zero_padding&0x1) ? o_tile_h                  : i_tile_h;
          tile.ih = (current_conv.zero_padding&0x1) ? o_tile_h + dil_h*(fw_h-1) : i_tile_h + dil_h*(fw_h-1);
...
_dbgmw_printf_ ("  tile.ibase_addr: %x \n", tile.ibase_addr);          

/*       in_row_made += tile.ih - ((fw_h-1) + (dil_h-1)*(1-!(ih>1))/2);                             //!!!!!!!!!!!!! DA VERIFICARE APPROFONDITAMENTE !!!!!!!!!!!!!!*/
       
       in_row_made += tile.ih - dil_h*(fw_h-1);  
       
_dbgmw_printf_ ("  in_row_made: %d \n", in_row_made); 
      ...
      // printf ("  tile.oh: %d  out_row_made: %d \n", tile.oh, out_row_made);
       ...
       tile.isize_B = tile.iw*BxP * tile.ih; // * N_COL;
                ...
                */
                ...
                for (i=0;i<num_burst;i++){
                  ...
                  for (tr=0;tr<N_COL;tr++){  
                    mchan_lin_transfer  (0, tile.isize_B, RX, INC, x_ptr_int, (unsigned int)(x_seq*(BYTE_PER_BANK/2) + (i*N_COL+tr)*BYTE_PER_BANK));
                    x_ptr_int += if_stride;
                ...
                mchan_lin_multi_transfer  (0, tile.isize_B, RX, INC, x_ptr_int, (unsigned int)(x_seq*(BYTE_PER_BANK/2)), if_stride, feat_per_IG);
                    x_ptr_int += if_stride*feat_per_IG;
#endif   

#ifdef MCHAN_2D
                mchan_twd_transfer        (0, tile.isize_B, RX, x_ptr_int, (unsigned int)(x_seq*(BYTE_PER_BANK/2)), if_stride, BYTE_PER_BANK, feat_per_IG,
                                                                                  0x00000000, 1, feat_per_IG);
                    x_ptr_int += if_stride*feat_per_IG;
#endif            
...
#ifdef MCHAN_2D 
                mchan_twd_transfer(1, 
                                   WS_SIZE*BxWei*n_OG*interl_fact, 
                                   RX, 
                                   w_ptr_int,  
                                   0x10000000 + x_seq*WEI_MEM_BANK_SIZE_B/2, 
                                   WS_SIZE*BxWei*n_OG*interl_fact, 
                                   WEI_MEM_BANK_SIZE_B*interl_fact, 
                                   (WL_PORTS/interl_fact),
                                   0x00000000, 
                                   1, 
                                   (WL_PORTS/interl_fact));
                w_ptr_int += WS_SIZE*BxWei*n_OG*WL_PORTS;
...
#else                  
			mchan_barrier_mw(0, dma_id[0]);  // DMA BARRIER 
#endif
                    _dbg_flag_( 0x72);        
 ...
 //  modelsim_wave_flag( 0x49);                 
                    ...
                    */
                    ...
                    if (ig==IG-1) { //add a store to queue only if the OG is complete
                      ...
                      unsigned int store_batch = n_OG / YSLOTSxSUBPART;
...
#else
			store_queue [sg_mod2][w_ptr[sg_mod2]].ext_ptr  = tile.obase_addr + (sg*current_conv.MAX_OG + i*YSLOTSxSUBPART) * og_stride;
#endif
...
//printf("yseq: %d\n", y_seq);               
 ...
 //                while (hwce_status()){;} // TODO: 
  //         ; // wait the end of the last convolution  
		...
		_dbg_flag_( 0x811);
#else
		mchan_barrier_mw(0, dma_id[0]); // wait for the transfers of all the remaining og in the partition that is going to be used
#endif
