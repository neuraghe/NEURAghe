...
float new_exp (float exp){
	int i, exp_int;

	exp_int= (int)ceil_new(exp);
    ...
    **/
    ...
    ///Softmax function
        ...
        for(i=0;i<layer->n_out_feat;i++){
        	sum+=new_exp(conversion_data_t_to_float(*(y+i),16));
        	printf ("sum: %f; exp: %x->%f; e^exp: %f\n", sum, *(y+i), conversion_data_t_to_float(*(y+i),16), new_exp(conversion_data_t_to_float(*(y+i),16)));
        }
        for(i=0;i<layer->n_out_feat;i++){
        	*(y+i)=conversion_float_to_data_t(new_exp(conversion_data_t_to_float(*(y+i),16))/sum);
        }
    #else
