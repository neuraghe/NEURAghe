  ...
  */
   ...
   begin
      ...
      begin
        ...
        if(change_feat_i[0])begin // yout
         ...
         weight_offset<=weight_offset  + (32/8)*(CONV_WIDTH/8); // dipende da word_num e weight_loader
  ...
  generate
    ...
    //   assign stream_config[j+2*N_ROW].base_addr   = reg_file.hwpe_params[8+3][31: 0] + BANK_SIZE*4*j;
       ...
       assign stream_config[j+2*N_ROW].base_addr   =  (input_buffer) ? BANK_SIZE*4/2 + BANK_SIZE*4*j : BANK_SIZE*4*j;
    end
  endgenerate
...
// assign wl_base_addr_o                =       weights_addr;// reg_file.hwpe_params[PARAMS_STREAM] + weight_offset; //weights
  ...
  assign wl_base_addr_o                =        (input_buffer) ? WEI_MEM_BANK_SIZE/2 + weight_offset : weight_offset; //weights
