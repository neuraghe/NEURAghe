...
#ifdef _DEBUG_MAIN_
	#define _dbgm_printf_(...) printf(__VA_ARGS__)
#else
...
#define WEI_MEM_BANK_SIZE 2048 //1024////2048//4096
...
#define WEI_MEM_BANK_SIZE_B (WEI_MEM_BANK_SIZE*8)
...
#define MCHAN_TRANSF_SIZE  4096// //32767 //65535 //16384
                                               ...
                                               unsigned int dummy, unsigned int num_batch_stores, unsigned int batch_size)
{
 ...
 int single_tr_len = single_tr_len_b>>3;
...
//      do
//      {
//        max_prog_size = batch_size*(single_tr_len/2);
//        num_tr_in_batch= num_tr_in_batch * 2;
//        dummy = dummy/2;
//        num_batch_stores = num_batch_stores*2;
//        last_prog_size = last_prog_size/2;
 ...
 *(volatile int*) mchan_reg = single_tr_len_b | (ext_stride_bis << 15);
 *(volatile int*) mchan_reg = single_tr_len_b | (tcdm_stride_bis << 15);
 
 if(id==0){
   *(volatile int*) mchan_reg = dummy | (num_tr_in_batch << 16);
   *(volatile int*) mchan_reg = last_prog_size | (num_batch_stores << 16);
...
static inline void mchan_twd_wei_transfer( unsigned int ext_addr,   unsigned int tr_piece_ddr_B,  unsigned int ext_stride, 
                                           unsigned int tcdm_addr,  unsigned int tr_piece_tcdm_B, unsigned int tcdm_stride,
                                           unsigned int tot_transf_size_B)
{
 
 int mchan_reg = MCHAN_WEI_COMMAND_QUEUE;
 
 
 int tot_64bit_words = tot_transf_size_B>>3;// the total number of 64 bit words to be transferred  
 
  
 *(volatile int*) mchan_reg = tot_64bit_words | (RX<<PLP_DMA_TYPE_BIT) | ( 1<<PLP_DMA_INCR_BIT) | 
                                              (1 <<PLP_DMA_2D_BIT) | (0<<PLP_DMA_ELE_BIT) | (0 <<PLP_DMA_ILE_BIT) | (0 <<PLP_DMA_BLE_BIT) | 
                                              (1 << PLP_DMA_2D_TCDM_BIT);
 ...
 *(volatile int*) mchan_reg = tr_piece_ddr_B  | (ext_stride << 15);
 *(volatile int*) mchan_reg = tr_piece_tcdm_B | (tcdm_stride << 15);
                                               ...
                                               unsigned int bank_slot_size_y, unsigned int num_batch_stores, unsigned int batch_size)
{
 ...
 int total_tr_size = tot_transf*single_tr_len;
    ...
    //printf("\ntotal_tr_size, %x, tot_transf %x, single_tr_len %x\n",total_tr_size,tot_transf,single_tr_len);
...
inline 
 ...
 void hwce_setup(unsigned char input_buffer, unsigned int y_in_ptr, unsigned int y_out_ptr, int part_base_addr, int last_addr, int slot_size,
                              unsigned int n_og, Tile tile, unsigned int width_hwce, short int fw_eff, short int qf, unsigned int precision8, 
                              short int sum_over_constant, short int rectifier_activ, int pooling_hw) {

	int conv_w = (precision8) ? 8 : 16;
...
// width is scaled down by a factor of two because image is 16-bit based
  ...
  //  hwce_set_IOReg       (HWCE_PARAMS_START_REG + 0, weight_ptr); // pointer to the weights array
    ...
    hwce_set_IOReg       (HWCE_PARAMS_START_REG + 1, (fw_eff << 16) | ((0x0000ffff) & 0)); // effective size of filter and number of mask
    
    hwce_set_IOReg       (HWCE_PARAMS_START_REG + 2, ((qf&0xF) << 28) | ((precision8&0x1) << 26)  | ((0&0x1) << 25)  |  (pooling_hw&0xF) <<21 |(tile.nb_nt_zp&0x7) << 18 | (input_buffer&1) << 15 |
                         (sum_over_constant&1) << 14 |  (rectifier_activ&1) << 13 | ((width_hwce/PxW)&0xFFF));
  ...
  */
     ...
     A+=uno;
      ...
      hwce_set_line_params          (0, (owidth_w)/pool_div+uno, (4*A));  // words, bytes
      hwce_set_feat_params          (0, tile.oh/pool_div  , (4*A*B));   // rows, bytes

 // Yin streams
      hwce_set_base_addr            (1,  y_in_ptr );                                      // hwce_set_base_addr  
      hwce_set_trans_size           (1,  n_og*tile.oh*owidth_w);  //words
      hwce_set_line_params          (1,  owidth_w , 4*owidth_w); // words, bytes
      hwce_set_feat_params          (1,  tile.oh   , 4*tile.oh*owidth_w);// rows, bytes
  ...
  //    hwce_set_base_addr            (2,  x_in_ptr );     
    //      hwce_set_IOReg                (11, x_in_ptr);                                             // hwce_set_base_addr  
      ...
      hwce_set_line_params          (2,  iwidth_w , 4*iwidth_w); // words, bytes
      hwce_set_feat_params          (2,  tile.ih   , 4*tile.ih*iwidth_w);// rows, bytes
