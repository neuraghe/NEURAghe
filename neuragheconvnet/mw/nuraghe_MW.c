 ...
 */
       ...
       current_conv.y_ext = 0x80000000;    
         ...
         conv_w = (current_conv.precision8) ? 8 : 16; //CONV_WIDTH; //
        ...
        // YS2=YS*2;
         ...
         if (BANK_SLOT_SIZE_Y%4)
           BANK_SLOT_SIZE_Y = BANK_SLOT_SIZE_Y -BANK_SLOT_SIZE_Y%(4);
         _dbgm_printf_("slotsize %d\n", BANK_SLOT_SIZE_Y);
         
         SLOT_SIZE_X = BYTE_PER_BANK/2;
         int img_size_B = current_conv.IH*current_conv.IW*BxP;
         
         TILEs = (int)(img_size_B/BANK_SLOT_SIZE_Y);      
         if (img_size_B%BANK_SLOT_SIZE_Y>0) TILEs++;  
         ...
         do{                    
           mid_tile_xh=current_conv.IH/TILEs+current_conv.FS-1;
         ...
         OW= current_conv.IW-(current_conv.FS-1)*(1-current_conv.zero_padding);
         OH= current_conv.IH-(current_conv.FS-1)*(1-current_conv.zero_padding);
         ...
         */     
         ...
         if (current_conv.IH/TILEs<current_conv.FS){
           printf ("ERROR! The tiling leads to a tile input height smaller than the filter size.\n");  
         ...
         if (current_conv.MAX_OG*N_ROW*N_COL*32*2*2>WEI_MEM_SIZE_B){
           printf ("ERROR! The weights group size (%d B) execeded the available weights memory (%d B).\n", current_conv.MAX_OG*N_ROW*N_COL*32*2*2, WEI_MEM_SIZE_B);  
           printf ("\t Try reducing MAX_OG value. Current value: %d\n", current_conv.MAX_OG);
   ...
   //printf("\n\nsetup:    %d\t  cycles\n\r", get_time());
         ...
         hwce_offload_ext ( current_conv.IH, current_conv.IW, current_conv.FS);  
...
void hwce_offload_ext ( int ih, int iw, int fw) {
     ...
     Store store_queue [2] [200];
      ...
      ow = (current_conv.zero_padding&0x1) ? iw: (iw-fw+1);
      oh = (current_conv.zero_padding&0x1) ? ih: (ih-fw+1);
    ...
    //   int tile_height = ih/TILEs; //unused
      ...
      */
        ...
        if (TILEs==1) { //only one tile
          ...
          tile.oh = o_tile_h;
          tile.ih = (current_conv.zero_padding&0x1) ? o_tile_h + (fw-1)/2 : o_tile_h + (fw-1);
...
#ifdef MCHAN_2D
                mchan_twd_transfer        (0, tile.isize_B/N_COL, RX, x_ptr_int, (unsigned int)(x_seq*(BYTE_PER_BANK/2)), if_stride, BYTE_PER_BANK, num_burst*N_COL,
                                                                                  0x00000000, 1, num_burst*N_COL);
                    x_ptr_int += if_stride*N_COL*num_burst;
#endif            
...
#ifdef MCHAN_2D 
                mchan_twd_transfer(1, WS_SIZE*BxWei*n_OG, RX, w_ptr_int,  0x10000000 + x_seq*WEI_MEM_BANK_SIZE_B/2, WS_SIZE*BxWei*n_OG, WEI_MEM_BANK_SIZE_B, WL_PORTS,
                                    0x00000000, 1, WL_PORTS);
                w_ptr_int += WS_SIZE*BxWei*n_OG*WL_PORTS;
          ...
          //modelsim_wave_flag( 0x49);  
              ...
              if (ig==IG-1) {//activation and pooling, if any, must be applied on the last IG only  
                ...
                pool_hw_on_last_IG = current_conv.pooling_hw;
                    ...
                    _dbghwce_printf_("                  ih=%d, iw=%d, fw=%d. sum_const=%d\n", tile.ih, tile.iw, fw, sum_over_constant);
                    ...
                    //  (unsigned int) BYTE_PER_BANK/2   * x_seq ,   //x_addr    [x_seq],
                                     ...
                                     n_OG,              // number of OG.                                      
                                     tile,
                                     iw,                // width_hwce, effective width processed in the specific offload
                                     fw,                // fw_eff
                                     current_conv.qf,   // qf
...
//printf("yseq: %d\n", y_seq);               
                        ...
                        if (w_ptr[sg_mod2]>=current_conv.MAX_OG-1)
                          w_ptr[sg_mod2]=0;
                        else
                          w_ptr[sg_mod2]++;
