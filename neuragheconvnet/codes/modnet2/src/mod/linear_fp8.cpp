...
static inline RET linear_fp8_forward_sw_core(signed char* input, signed char* output, SIZE in_s, SIZE out_s,
        signed char* weights, signed char* bias, int qf) {

#ifdef _ARMTODOOOOOOOOOO_
    ...
    for (ITER hkern = 0; hkern < out_s; hkern++) {
        ...
        output[hkern] = (signed char)saturate(mac >> qf, "linear");
    } 
#endif
    return OK;
}
...
LINEAR_FP8 linear_fp8_create(void) {
    LINEAR_FP8 lin = (LINEAR_FP8) calloc(1, sizeof(struct Linear_fp8));
    return lin;
}
...
RET linear_fp8_init(LINEAR_FP8 lin, NAME weightsFile, NAME biasFile, SIZE in_s,
    ...
    lin->weights = (signed char*) calloc(weight_s, sizeof(signed char));
    lin->bias = (signed char*) calloc(bias_s, sizeof(signed char));

    res = load_f62tofp8_from_file(lin->weights, weightsFile, weight_s);
    ASSERT(res == OK, "%s", "loading kernel tensor failed");

    res = load_f62tofp8_from_file(lin->bias, biasFile, bias_s);
...
RET linear_fp8_forward(LINEAR_FP8 lin, signed char* __restrict__ input, signed char* __restrict__ output, SIZE in_s,
        SIZE out_s) {
    ...
    _tcreate_(time);
    linear_fp8_forward_sw_core(input, output, in_s, out_s, lin->weights,
            lin->bias, lin->qf);
