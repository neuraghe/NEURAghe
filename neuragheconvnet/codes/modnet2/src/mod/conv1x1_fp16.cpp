...
static inline RET linear_fp16_forward_sw_core(int16_t* input, int16_t* output, SIZE in_s, SIZE out_s,
    ...
    for (uint32_t hkern = 0; hkern < out_s; hkern+=NEON_BLOCK>>1) {
        ...
        for (uint32_t iblock = 0; iblock < 2; iblock++){
            ...
            for (uint32_t px = 0; px < in_s; px+=NEON_BLOCK){
                
                int16x8_t w_block;
                int16x8_t w_block1;
                int16x8_t in;
                
                w_block0 = vld1q_s16(&weights[hkern_offset0 +px]);
                w_block1 = vld1q_s16(&weights[hkern_offset1 +px]);
                in       = vld1q_s16(&input[px]);

                __builtin_prefetch(&input[px+PREFETCH_OFFSET]);
