...
static inline RET maxpool_fp16_forward_sw_core(int16_t* input, int16_t* output, SIZE in_s[3],
	...
	/* for each plane */
	//printf ("for pooling: %lx %lx\n",(long unsigned int)input,(long unsigned int)output);
	//#pragma omp parallel for firstprivate(_stride0, _stride1, _of, _of_h, _of_w, _if, _if_h, _if_w, _k_h, _k_w, _pad_h, _pad_w) private(hout,wout) collapse(2)		
