...
#define SVD_CROP 512
DATA fc6svd_weights_array[512*7*7*SVD_CROP] __attribute__ ((aligned (16)));
DATA fc6svd_bias_array[SVD_CROP] __attribute__ ((aligned (16)));
DATA fc6_weights_array[SVD_CROP*4096] __attribute__ ((aligned (16)));
...
DATA fc6svd_output[SVD_CROP] __attribute__ ((aligned (16)));
...
void cnnMainInit(VARNAME load_data_dir)
{
	...
	/* Conv Buffers */
	conv_input[0] = (DATA*)neu_DDRmalloc(sizeof(DATA)*64*224*224);
	conv_input[1] = (DATA*)neu_DDRmalloc(sizeof(DATA)*64*224*224);
	
	/* Conv Buffers */
	conv_output[0] = (DATA*)neu_DDRmalloc(sizeof(DATA)*512*8*7);
	conv_output[1] = (DATA*)neu_DDRmalloc(sizeof(DATA)*512*8*7);

	wPointer = (DATA*)neu_DDRmalloc(sizeof(DATA));
	 ...
	 * num outputs = SVD_CROP
	 * W SVD_CROP x 25088
	 * X 25088 x 1
	 */
	SIZE fc6svd_dim = SVD_CROP;
	fc6svd_param = linear_fp16_create();
	fc6svd_param->out_s = fc6svd_dim;
	fc6svd_param->in_s = conv5_3_dim;
	sprintf(filename, "%s/fc6_weights_array_file", load_data_dir);
	load_fixed(filename,conv5_3_dim*fc6svd_dim,fc6svd_weights_array);
	fc6svd_param->weights = fc6svd_weights_array;
	sprintf(filename, "%s/fc6_bias_array_file", load_data_dir);
	load_fixed(filename,fc6svd_dim,fc6svd_bias_array);
	fc6svd_param->bias = fc6svd_bias_array;
	...
	fc6_param->in_s = fc6svd_dim;
	sprintf(filename, "%s/fc6_weights_array_file", load_data_dir);
	load_fixed(filename,fc6svd_dim*fc6_dim,fc6_weights_array);
	fc6_param->weights = fc6_weights_array;
	sprintf(filename, "%s/fc6_bias_array_file", load_data_dir);
	load_fixed(filename,fc6_dim,fc6_bias_array);
...
void cnnMain(DATA* image, float* results)
{
	...
	if(stage_id==0){
		interlace(conv_input[0], input_output, 3, input_size[0], input_size[1], input_size[2]);
		stage_id++;
		return;
	}

	job_id = spatconv_forward_hw(conv1_1_param, conv_input[!(stage_id%2)], soc->out, soc, input_size, conv1_1_size, conv1_1_stride, conv1_1_pad, true);
	...
	soc->out = (DATA*)((uintptr_t)soc->out ^ (uintptr_t)soc->in);
	soc->in = (DATA*)((uintptr_t)soc->in ^ (uintptr_t)soc->out);
	job_id = basicBlock_hw(conv5_3_param, soc->in, conv_output[!(stage_id%2)], soc, conv5_2_size, conv5_3_size, conv5_3_stride, conv5_3_pad, POOL_2x2, MAX_POOL, true);
	...
	if(stage_id==1){
		//Interlace next frame
		interlace(conv_input[(stage_id%2)], input_output, 3, input_size[0], input_size[1], input_size[2]);

		spatconv_wait(soc, job_id);
		//printf("CHECK: %lld \n", checksum(conv_output[!(stage_id%2)], 512*8*7));
	...
	deinterlace(conv5_3_output, conv_output[(stage_id%2)], conv5_3_size[0], conv5_3_size[1], conv5_3_size[2], conv5_3_output_conv5_3_output_dstride);

	linear_fp16_forward(fc6svd_param, conv5_3_output, fc6svd_output, conv5_3_dim, fc6svd_param->out_s);
	linear_fp16_forward(fc6_param, fc6svd_output, fc6_output, fc6_param->in_s, fc6_param->out_s);
