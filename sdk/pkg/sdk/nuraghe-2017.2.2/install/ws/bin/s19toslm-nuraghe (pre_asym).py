...
word_size    = 8 # word size of the smaller port: 4 Bytes or 8 bytes
...
l2_bank_size = 4096 # in words (64 bit)
l2_banks     = mem_depth / l2_bank_size

l2_start     = 0x1C000000
l2_end       = l2_start + l2_banks * l2_bank_size * word_size - 1
...
for addr in sorted(slm_dict.keys()):
    ...
    if(addr >= l2_start and addr <= l2_end):
        ...
        cut     = l2_base >> (int(math.log(l2_bank_size, 2) + (word_size/4-1)))
        byte    = addr % 8
        
       # if (cont<10):
       #   print ("addr: %x, data: %s" %(l2_base, data))
       # cont=cont+1
        
        l2_addr = (l2_base >> (word_size/4-1)) % l2_bank_size
        l2_size += 1
        for b in range(0, 4):
          if (l2_base % (word_size/4)==0):
            l2_cut_files[cut][b  ].write("@%08X %02x\n" % (l2_addr, (data>>(b*8))&0xFF))
          else:
