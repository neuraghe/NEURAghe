...
static inline void flush_L0_buffers(int conf){
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x0CU) = conf;
    return;
}
...
static inline unsigned int read_hit_icache_reg(unsigned int bank){
    ...
    reg_status = *(volatile unsigned int *) (ICACHE_CTRL_BASE_ADDR+0x18U+bank*0x04U);
    return reg_status;
}
...
static inline unsigned int read_trans_icache_reg(unsigned int bank){
    volatile unsigned int reg_status;
    reg_status = *(volatile unsigned int *) (ICACHE_CTRL_BASE_ADDR+0x28U+bank*0x04U);
    return reg_status;
}
...
static inline void flush_all_icache_banks(void){
    volatile int num_core = plp_nbCores();
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x04U) = (1<<num_core)-1;// write 4'b1111 in the flush reg
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR) = 0x0;// disable the icache banks
    
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR) = (1<<num_core)-1;// enable the icache banks
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x04U) = 0x0;// write 4'b000 in the flush reg
    return;
}
...
static inline int read_status_icache_reg(void){
    volatile int reg_status;
    reg_status = *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x08U);
    return reg_status;
}

static inline void flush_some_icache_banks(int conf){
    volatile int current_conf = *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x08U);
...
static inline void set_icache_private(void){
    ...
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x04U) = (0x1<<num_core)-0x1;// write 4'b1111 in the flush reg
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR) = 0x0;// disable the icache banks
    
    //Drive the icache_is_private signal
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0xF0U) = 0x1;
    
    //Enable all icache banks
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR) = (0x1<<num_core)-0x1;// enable the icache banks
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0x04U) = 0x0;// write 4'b000 in the flush reg
    return;
}
...
static inline void set_icache_shared(void){
    ...
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR) = 0x0;// disable the icache banks
    ...
    //Drive the icache_is_private signal
    *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0xF0U) = 0x0;
...
static inline int read_icache_pri_sh_reg(void){
    volatile int reg_status;
    reg_status = *(volatile int*) (ICACHE_CTRL_BASE_ADDR+0xF0U);
    return reg_status;
}
