...
#ifndef __HAL_EU_V3_H__
#define __HAL_EU_V3_H__
...
{
  return pulp_read32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_BUFFER);
}
...
{
  return pulp_read32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_BUFFER_MASKED);
}
...
{
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_BUFFER_CLEAR, evtMask);
}
...
{
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_MASK, evtMask);
}
...
{
  eu_evt_maskSet_base(EVENT_UNIT_DEMUX_BASE_ADDR, evtMask);
}
...
#ifdef __riscv__
  // TODO riscv compiler is not able to factorize the event unit base if we use classic C code
  __asm volatile ("sw %0,%1(%2)" : : "r" (evtMask), "I" (EU_CORE_MASK_AND), "r" (EVENT_UNIT_DEMUX_BASE_ADDR) );
#else
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_MASK_AND, evtMask);
#endif
}
...
{
  return evt_read32(EVENT_UNIT_DEMUX_BASE_ADDR, EU_CORE_EVENT_WAIT);
}
...
{
  return evt_read32(EVENT_UNIT_DEMUX_BASE_ADDR, EU_CORE_EVENT_WAIT_CLEAR);
}
...
{
  return pulp_read32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_BUFFER_IRQ_MASKED);
}

static inline void eu_irq_mask(unsigned int irqMask)
{
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_MASK_IRQ, irqMask);
}
...
{
  eu_irq_maskSet_base(EVENT_UNIT_DEMUX_BASE_ADDR, irqMask);
}

static inline void eu_irq_maskClr(unsigned int irqMask)
{
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_CORE_MASK_IRQ_AND, irqMask);
}
...
{
  return BBMUX_CLKGATE_BASE_ADDR + EU_SW_EVENTS_AREA_BASE + EU_CORE_TRIGG_SW_EVENT + (event << 2);
}

static inline unsigned int eu_evt_trig_cluster_addr(int cluster, int event)
{
  return PULP_CLUSTER_BASE_ADDR(cluster) + BBMUX_CLKGATE_OFFSET + EU_SW_EVENTS_AREA_BASE + EU_CORE_TRIGG_SW_EVENT + (event << 2);
}

static inline unsigned int eu_evt_trig_addr(int event)
{
  return EVENT_UNIT_DEMUX_BASE_ADDR + EU_SW_EVENTS_DEMUX_OFFSET + EU_CORE_TRIGG_SW_EVENT + (event << 2);
}
...
{
  return EVENT_UNIT_DEMUX_BASE_ADDR + EU_BARRIER_DEMUX_OFFSET + EU_BARRIER_AREA_OFFSET_GET(barId);
}
...
{
  return pulp_read32(BBMUX_CLKGATE_BASE_ADDR + EU_SOC_EVENTS_AREA_OFFSET + EU_SOC_EVENTS_CURRENT_EVENT);
}
...
{
  return EVENT_UNIT_DEMUX_BASE_ADDR + EU_MUTEX_DEMUX_OFFSET + (mutexId << 2);
}
...
{
  return evt_read32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_DISPATCH_DEMUX_OFFSET, EU_DISPATCH_FIFO_ACCESS);
}

static inline unsigned int eu_dispatch_push(unsigned value)
{
  return pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_DISPATCH_DEMUX_OFFSET + EU_DISPATCH_FIFO_ACCESS, value);
}

static inline void eu_dispatch_team_config(unsigned value)
{
  pulp_write32(EVENT_UNIT_DEMUX_BASE_ADDR + EU_DISPATCH_DEMUX_OFFSET + EU_DISPATCH_TEAM_CONFIG, value);
}
...
{
  return EVENT_UNIT_DEMUX_BASE_ADDR + EU_LOOP_DEMUX_OFFSET;
}
...
{
  return BBMUX_CLKGATE_BASE_ADDR + EU_CORE_AREA_OFFSET_GET(coreId);
}

static inline unsigned int eu_core_base(int coreId)
{
  return BBMUX_CLKGATE_BASE_ADDR + EU_CORE_AREA_SIZE * coreId;
}
