...
#ifndef __HAL_MCHAN_V5_H__
#define __HAL_MCHAN_V5_H__

#include <archi/mchan_v5.h>
#include "hal/pulp.h"

#define MCHAN_COMMAND_QUEUE (MCHAN_BASE_ADDR + MCHAN_COMMAND_QUEUE_OFFSET)
#define MCHAN_STATUS_REGISTER (MCHAN_BASE_ADDR + MCHAN_STATUS_REGISTER_OFFSET)
...
static inline void pulp_dma_barrier() {
  unsigned int addr = MCHAN_BASE_ADDR + MCHAN_STATUS_REGISTER_OFFSET;
...
static inline void pulp_dma_wait(unsigned int dmaBase, unsigned int counter) {
  pulp_dma_wait_event(dmaBase, counter, PULP_DMA_EVT);
}
