...
/* This example shows the usage of task-based UDMA transfers from I2S to L2 using plp_udma_enqueueCopy_l2.
...
static void udma_task(unsigned int arg) {
  ...
  if (enqueued < NB_ITER) {
    enqueued++;
    plp_periph_copy_toL2(&udmaCopy[current], PLP_UDMA_I2S0_CHANNEL, (int) l2Buffers[current], BUFSIZE, UDMA_CHANNEL_CFG_SIZE_32, plp_task(udma_task, 0));
  }
...
static void test_task(int arg) {
  ...
  plp_periph_copy_toL2(&udmaCopy[0], PLP_UDMA_I2S0_CHANNEL, (int) l2Buffers[0], BUFSIZE, UDMA_CHANNEL_CFG_SIZE_32, plp_task(udma_task, 0));
  plp_periph_copy_toL2(&udmaCopy[1], PLP_UDMA_I2S0_CHANNEL, (int) l2Buffers[1], BUFSIZE, UDMA_CHANNEL_CFG_SIZE_32, plp_task(udma_task, 0));
}
